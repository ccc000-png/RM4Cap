...
class Translator(object):
    ...
    def __init__(self, checkpoint, model=None):
        self.max_t_len = checkpoint['model_config'].cfgs.decoder.max_caption_len
        self.PAD = 0
        self.BOS = checkpoint['model_config'].vocab_size - 2
    ...
    def translate_batch_single_sentence_greedy(self, feature2ds, input_ids, input_mask, pair,model):
        ...
        for dec_idx in range(max_t_len):
            ...
            outputs = model(feature2ds, input_ids, input_mask, pair)
    ...
    def translate_batch(self, feature2ds, input_ids, input_mask, pair):
        """while we used *_list as the input names, they could be non-list for single sentence decoding case"""
        return self.translate_batch_single_sentence_greedy(feature2ds, input_ids, input_mask, pair, self.model)
...
def run_translate(data_loader, translator, device,epoch, opt):
    ...
    for cur_step, (
            ...
            video_id, pair) in enumerate(data_loader):
        ...
        dec_seq = translator.translate_batch(feature2ds, input_ids, input_mask, pair)
