...
class CaptionHead(nn.Module):
    def __init__(
            ...
            vocab_size,fusion_sem,fusion_fine
    ):
        super(CaptionHead, self).__init__()
        # sentence
        self.fusion_sem=fusion_sem
        self.fusion_fine=fusion_fine
    ...
    def Fusion(self, visual, sem_features, fine_features,fusion_sem=True,fusion_fine=True,):
        if fusion_sem:
            U_objs = self.Uo(sem_features)
            attn_feat = visual.unsqueeze(2) + U_objs + self.bo  # (bsz, sample_numb, max_objects, hidden_dim)
           ...
           features = visual
        if fusion_fine:
            U_motion = self.Um(fine_features)
    ...
    def forward(self, visual, objects, actions, input_ids, input_mask):
        ...
        context_feats = self.Fusion(visual, objects, actions,self.fusion_sem,self.fusion_fine)
