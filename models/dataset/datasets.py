...
class CaptionDataset(Dataset):
    def __init__(self, cfgs, mode):
        ...
        """3.获取辅助特征"""
        image_text_path = "/media/hpc/39C3AC34579106FA/CX/VLTinT/preprocess/Image_Text/clip_l14_infer6/frame_feature2"
        self.image_texts=[]
        for pair in tqdm(os.listdir(image_text_path)):
            with open(os.path.join(image_text_path,pair),"rb") as f:
                image_text = pickle.load(f)
                self.image_texts+=image_text
    ...
    def __getitem__(self, idx):
        ...
        # new_image_text=[]
        # for pair in tqdm(self.image_texts):
        #     cls_token = pair["text"]
        #     cls_token_ids = clip.tokenize(cls_token, context_length=3, truncate=True)[0]
        #     new_image_text+=[{"text": cls_token_ids, "image": pair["image"]}]
        pair=self.image_texts
        ...
        return torch.FloatTensor(feature2d),\
               caption_ids, caption_mask, caption_labels, captions, video_id,pair

def collate_fn_caption(batch):
    feature2ds, caption_ids, caption_mask, caption_labels, captions, video_id, pair = zip(*batch)
    ...
    return feature2ds.float(), caption_ids.long(),  caption_labels.float(), caption_masks.float(),captions, video_id,pair
